  ...
  for (const ph of placeholders) {
    ...
    if (typeof value === 'string') {
      console.log(`🔤 Case formatting for ${ph.originalKey}:`, {
        ...
        isAllCaps: ph.originalKey === ph.originalKey.toUpperCase() && ph.originalKey.includes(' '),
        isAllLower: ph.originalKey === ph.originalKey.toLowerCase(),
        isTitleCase: ph.originalKey === ph.originalKey.charAt(0).toUpperCase() + ph.originalKey.slice(1).toLowerCase(),
        value: value
      });
      ...
      if (ph.originalKey === ph.originalKey.toUpperCase() && ph.originalKey.includes(' ')) {
        ...
        console.log(`  → Title case formatting: ${value} → ${formattedValue}`);
      } else {
...
export async function generatePlaceholderForm(placeholders: PlaceholderField[], templateName: string): Promise<void> {
  const pdfBytes = await generatePlaceholderFormBytes(placeholders, templateName);
  
  // Save the PDF
  console.log('Saving PDF...');
  console.log('PDF bytes generated:', pdfBytes.length);
  const blob = new Blob([new Uint8Array(pdfBytes)], { type: 'application/pdf' });
  const filename = `${templateName}_Placeholder_Form.pdf`;
...
export async function generatePlaceholderFormBytes(placeholders: PlaceholderField[], templateName: string): Promise<Uint8Array> {
  try {
    ...
    addText(introText, margin, yPosition, 12);
    yPosition -= 30; // Simple spacing after intro
    ...
    for (const group of sortedPlaceholders) {
      ...
      yPosition -= 25; // Simple spacing after group heading
      ...
      for (const placeholder of group.placeholders) {
        ...
        yPosition -= 8; // Spacing after label
        
        // Create fillable text field
        const fieldName = `field_${placeholder.key.replace(/[^a-zA-Z0-9]/g, '_').replace(/_+/g, '_').replace(/^_|_$/g, '')}`;
        
        // Skip if field name already exists
        if (usedFieldNames.has(fieldName)) {
          console.warn(`Skipping duplicate field: ${placeholder.key} -> ${fieldName} (already exists)`);
          continue;
        }
        ...
        yPosition -= 40; // More spacing between text boxes to prevent overlap
      }
      
      yPosition -= 15; // Simple spacing between groups
    }
    
    // Always add instructions on a separate page
    const instructionsPage = pdfDoc.addPage([612, 792]);
    let instructionsYPosition = height - margin;
    
    // Add some space at the top of instructions page
...
function sortPlaceholdersForForm(placeholders: PlaceholderField[]): Array<{title: string, placeholders: PlaceholderField[]}> {
  ...
  placeholders.forEach(placeholder => {
    ...
    if (/(investor|purchaser|buyer)/.test(key)) {
